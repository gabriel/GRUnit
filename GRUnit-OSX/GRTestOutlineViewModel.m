...
@implementation GRTestOutlineViewModel
...
- (id)outlineView:(NSOutlineView *)outlineView objectValueForTableColumn:(NSTableColumn *)tableColumn byItem:(id)item {
	...
	} else if ([[tableColumn identifier] isEqual:@"status"] && [[item children] count] == 0) {
...
- (void)outlineView:(NSOutlineView *)outlineView willDisplayCell:(id)cell forTableColumn:(NSTableColumn *)tableColumn item:(id)item {
	...
	GRTestNode *test = (GRTestNode *)item;
	...
	if ([[tableColumn identifier] isEqual:@"status"]) {
		...
		if ([test status] == GRTestStatusErrored) {
			[cell setTextColor:[NSColor redColor]];
		} else if ([test status] == GRTestStatusSucceeded) {
			[cell setTextColor:[NSColor greenColor]];
		} else if ([test status] == GRTestStatusRunning) {
